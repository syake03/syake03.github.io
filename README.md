...
## ポートフォリオとして守ること(2026-08-27に追加)

プロのポートフォリオと比べたときに欠けていた点を埋めた。**外すと素人っぽくなる。**

- **制作年と役割を必ず書く**(`.credit`)。「いつ・自分が何をしたか」が無い作品一覧は評価できない。
  takayaohta.com など、この業界の標準はどれも年とカテゴリを添えている。
  ラジオだけは**ふたり番組**なので「(ふたり番組)」と明記する — 単独制作に見せない
- **作品の画は広い画面で大きく見せる。** 900px以上で本文カラムから左右100pxはみ出させている。
  主役は画であって本文ではない。本文は読みやすい幅(620px)のまま
- **ダークでは作品カットを暗くする。** カットの地色(#EAE5D9)は、ダーク背景と**14.7:1**になり
  黒画面に光る板が並ぶ。`filter: brightness(.74)` で **7.9:1** まで落として馴染ませている
- カットは1500x990・JPEG品質62で書き出す(6枚で約500KB)。デスクトップで拡大表示するため
  1000pxでは足りない

## Safari だけ本文の右が空く(2026-09-02に踏んだ)

iPhone (Safari) で見ると、作品の説明文だけが画の右端より **6文字ぶん手前** で
毎行折り返し、右側に縦長の余白ができていた。Chrome では再現しない。

原因は `p { text-wrap: pretty; }`。Safari は pretty を「段落全体の最適化」として
実装していて、**最終行が2〜3文字で余るとき、それを避けるために全行を均等に
短く組み直す**。日本語は1文字ごとに折り返せるので、この均し方の影響が大きい。
Sumi の説明文は自然に組むと 4行目が3文字(43px)だったため、Safari が
19/19/19/18文字に均し、1行あたり83px(=23%)を捨てていた。画像は100%幅のまま
なので、本文だけが内側に寄って見える。

対処は2つセットで:

1. `p` から `text-wrap: pretty` を外す(見出しの `text-wrap: balance` は残す。
   1〜2行の見出しでは均すのが正しいので害がない)。
2. pretty が消えると最終行が1〜3文字で余るので、**説明文を3行に収まる
   65〜69文字に詰める**。本文は `--t-body` = 14.16px、字送りもほぼ14.16px なので、
   本文カラム幅 ÷ 14.16 × 3 が入る文字数の上限になる。

検証は目視ではなく `Range.getClientRects()` で行単位に測ること:

```js
const r=document.createRange(); r.selectNodeContents(p);
[...r.getClientRects()].map(b=>Math.round(b.width))   // 行ごとの幅
```

現状は本文カラム 346px以上(iPhone 390pt以上)で全6件が3行に収まる。
iPhone 375pt(カラム330px)だけ2件が2文字ほど余るが、
全行に余白が出るより軽い、と判断した。**説明文を書き換えるときは、
必ずこの測定をやり直すこと。**
